...
* [Jakarta EE](https://jakarta.ee){: new_window} ![外部鏈結圖示](../icons/launch-glyph.svg "外部鏈結圖示") 和 [Java&trade; EE](https://www.oracle.com/technetwork/java/javaee/overview/index.html){: new_window} ![外部鏈結圖示](../icons/launch-glyph.svg "外部鏈結圖示")，包含用於不同的規格（如 JNDI 或 JAX-RS）的特性。
...
在 [https://openliberty.io/guides/](https://openliberty.io/guides/){: new_window} ![外部鏈結圖示](../icons/launch-glyph.svg "外部鏈結圖示") 上，有很多實用的開發人員手冊，用於指導如何使用 Liberty 建立微服務和雲端本機應用程式。
...
Liberty 提供工具及最佳化支援，以將 Spring Boot fat Jar 轉換成已最佳化的壓縮 Docker 容器，利用快取的 Docker 映像檔層來改善循環和發佈時間。透過將很少變更的程式庫相依關係向下推送到個別的層中，並僅保留頂端中的應用程式類別，迭代重建和重新部署的速度會快得多。如需相關資訊，請參閱[為 Spring Boot 應用程式建立雙層 Docker 映像檔](https://openliberty.io/blog/2018/07/02/creating-dual-layer-docker-images-for-spring-boot-apps.html){: new_window} ![外部鏈結圖示](../icons/launch-glyph.svg "外部鏈結圖示")。
...
請查看 docker Hub 上的 [`websphere-liberty`](https://hub.docker.com/_/websphere-liberty/){: external} 或 [`open-liberty`](https://hub.docker.com/_/open-liberty/){: external}，以獲取基本映像檔的最新清單。

### Open Liberty 及 Docker
{: #openliberty-docker}

從 [Docker Hub 上的映像檔清單](https://hub.docker.com/_/open-liberty/){: new_window} ![外部鏈結圖示](../icons/launch-glyph.svg "外部鏈結圖示") 中選擇最適合應用程式的映像檔（例如，`open-liberty:microProfile`），並為您的服務建立一個 Dockerfile，其 `FROM` 該映像檔延伸而來。新增指令，以將應用程式及伺服器配置複製到新的映像檔。範例如下：
...
對於 Docker 映像檔，WebSphere Liberty 支援的基本自訂型樣與 Open Liberty 支援的相同。但是 Liberty 的固有模塊化設計使得建立包含應用程式及其所需特定特性集的自訂映像檔十分簡單（且典型）。WebSphere Liberty 有一個適用於無特性核心的映像檔 [`websphere-liberty:kernel`](https://github.com/WASdev/ci.docker/blob/9d28dfba4d20596f89b393bc9e3ae8295feec469/ga/developer/kernel/Dockerfile){: new_window}![外部鏈結圖示](../icons/launch-glyph.svg "外部鏈結圖示")，其為真正的自訂映像檔奠定了札實的基礎。

[`websphere-liberty` 映像檔檔案](https://hub.docker.com/_/websphere-liberty/){: new_window} ![外部鏈結圖示](../icons/launch-glyph.svg "外部鏈結圖示") 說明建立自訂映像檔時所需的下列簡單 3 行 Dockerfile：
...
建立自訂映像檔的更好方式是利用特性的明確清單來呼叫 `installUtility`。這個方法仍會建立最小映像檔，但在建置時期會變得更好，因為映像檔的層可以快取並重複使用，而不管伺服器配置的變更。例如，下列範例會建立使用特性子集的自訂映像檔，以支援使用 JAX-RS、JNDI 和 WebSocket 的應用程式：
...
1. 您要如何重複使用或自訂您的基礎映像檔？
    這些步驟會產生盡可能最小的映像檔，如果每個應用程式的特性不同，則有可能會犧牲重複使用能力。但是，具有小的自訂映像檔意味著應用程式可以在不同 Docker 主機上快速進行部署。如果您不確定，請從 Docker Hub 開始使用其中一個預先產生的映像檔，讓重複使用性達到最高。
2. 您多久一次更新此映像檔？
